            ...
            for(var i = 0 ; i < data.length; i++){
                for(var j = 0 ; j< length ; j++){
                    if (data[i].LICENSENO ==$('[name="licenseno"]:eq('+j+')').attr('value')){
                        $('[name="licenseno"]:eq('+j+')').parent().parent().find('[name="isdelete"]').val(data[i].ISDELETE).text(data[i].ISDELETE_).prop('selected',true);
                        $('[name="licenseno"]:eq('+j+')').parent().parent().find('[name="licensecnt"]').val(data[i].BUYCNT).text(data[i].BUYCNT).prop('selected',true);
